...
#from cheff.sr.diffusor_CFD import SR3Diffusor, SR3DDIMDiffusor
from cheff.sr.diffusor import SR3Diffusor, SR3DDIMDiffusor,PLMSDiffusor
...
class CheffSRModel:
    ...
    def sample(
        ...
        img_sr = diffusor.p_sample_loop(sr=img)   #change the input here
        img_sr.clamp_(-1, 1)
        img_sr = (img_sr + 1) / 2
        return img_sr
    
class CheffSRModel_PLMS:
    def __init__(
            self,
            model_path: str,
            device: Union[str, int, torch.device] = 'cuda'
    ...
    def sample(
        ...
        if method == 'ddim':
            ...
            diffusor = PLMSDiffusor(
                model=self.model,
                schedule=self.schedule,
            )

        img_sr = diffusor.p_sample_loop(sr=img)   # Adjust input here if necessary
        img_sr.clamp_(-1, 1)
        img_sr = (img_sr + 1) / 2
        return img_sr
