...
class PLMSDiffusor(Diffusor):
    """Class for modelling the sr process with PLMS."""
    ...
    def __init__(
        ...
        """Initialize PLMS sampler."""
        super().__init__(model, schedule, device, clip_denoised)
        self.sampling_steps = sampling_steps

        self.plms_timesteps = torch.arange(
            start=0,
            end=schedule.timesteps,
            step=schedule.timesteps // self.sampling_steps,
        )

        self.plms_alpha_cumprod = self.schedule.alphas_cumprod[self.plms_timesteps]
        self.plms_alphas_cumprod_prev = torch.cat(
            [
                self.schedule.alphas_cumprod[0].unsqueeze(0),
                self.schedule.alphas_cumprod[self.plms_timesteps[:-1]],
            ]
        )

    @torch.no_grad()
    def p_sample_plms(self, x: Tensor, sr: Tensor, t: Tensor, old_eps: list, index: int) -> Tuple[Tensor, Tensor]:
        """Sample from reverse process using PLMS."""
        b = x.shape[0]
        a_t = torch.full((b, 1, 1, 1), self.plms_alpha_cumprod[index], device=self.device)
        a_prev = torch.full((b, 1, 1, 1), self.plms_alphas_cumprod_prev[index], device=self.device)
        ...
        if len(old_eps) == 0:
            ...
            x_prev = a_prev.sqrt() * (x - sqrt_one_minus_at * e_t) / a_t.sqrt() + \
                     (1. - a_prev).sqrt() * e_t
            e_t_next = self.model(torch.cat([x_prev, sr], dim=1), t - 1)
        ...
        x_prev = a_prev.sqrt() * (x - sqrt_one_minus_at * e_t_prime) / a_t.sqrt() + \
                 (1. - a_prev).sqrt() * e_t_prime
    ...
    def p_sample_loop(self, sr: Tensor) -> Tensor:
        ...
        pbar = tqdm(
            iterable=torch.flip(self.plms_timesteps, dims=(0,)),
            desc='PLMS sampling loop time step',
            total=len(self.plms_timesteps),
            leave=False,
        )

        for i, step in enumerate(pbar):
            index = len(self.plms_timesteps) - i - 1
            t = torch.full((batch_size,), step, device=self.device, dtype=torch.long)
            img, e_t = self.p_sample_plms(img, sr, t, old_eps, index)
            old_eps.append(e_t)
            if len(old_eps) > 3:
                old_eps.pop(0)
        return img
    ...
    def p_sample_loop_with_steps(self, sr: Tensor, log_every_t: int) -> Tensor:
        ...
        for i, step in enumerate(pbar):
            ...
            if i % log_every_t == 0 or i == len(self.plms_timesteps) - 1:
                result.append(img)
