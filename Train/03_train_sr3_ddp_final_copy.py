...
class CustomSRDataset(Dataset):
    def __init__(self, root: str, transforms: Compose) -> None:
        ...
        self.sr_dir = os.path.join(root, 'lowres')
        self.hr_dir = os.path.join(root, 'lowres')
        self.mask_dir = os.path.join(root, 'lowres')
...
def run(rank: int, world_size: int, cfg: Dict) -> None:
    ...
    if rank == 0:
        wandb.init(project='fine-tuning-chexray-8th-aug-mask_ft', config=cfg)
...
def train(
    ...
    for ep in range(ep_start, epochs + 1):
        ...
        for batch in tqdm(train_loader, leave=False):
            ...
            #x_in = torch.cat([x_sr_noisy1, x_sr_noisy1], dim=1)
            ...
            with autocast():
                # Predict noise on HR image
                predicted_noise = diff.model(x_in,x_mask, t)
                # Compute loss
