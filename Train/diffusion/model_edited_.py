...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, dim_out: int, groups: int = 8) -> None:
        ...
        self.proj = nn.Conv2d(dim, dim_out, 3, padding=1)
         ...
         # Adjust groups if necessary
        if dim_out % groups != 0:
            groups = math.gcd(dim_out, groups)  # Find the greatest common divisor
...
#     """Adapter module to run in parallel with Unet and merge at the bottleneck/end."""

#     def __init__(self, dim, time_dim):
#         super().__init__()

#         # Define the adapter layers
#         self.residual_block1 = ResnetBlock(dim, dim, time_dim)
#         self.residual_block2 = ResnetBlock(dim, dim, time_dim)
#         self.transformer_block = PreNorm(dim, LinearAttention(dim))

#     def forward(self, x, t):
#         x1 = self.residual_block1(x, t)
#         x2 = self.residual_block2(x1, t)
#         x3 = self.transformer_block(x2)
...
class Adapter(nn.Module):
    def __init__(self, dim_in, dim_out, time_dim):
        ...
        self.residual_block1 = ResnetBlock(dim_in, dim_in, time_dim)
        self.residual_block2 = ResnetBlock(dim_in, dim_in, time_dim)
        self.transformer_block = PreNorm(dim_in, LinearAttention(dim_in))
        self.downsample_conv = get_downsample_conv(dim_in)
        self.final_conv = nn.Conv2d(dim_in, dim_out, 1)  # Add this line

    def forward(self, x, t):
        x1 = self.residual_block1(x, t)
        x2 = self.residual_block2(x1, t)
        x3 = self.transformer_block(x2)
        x4 = self.downsample_conv(x3)
        return self.final_conv(x4)  # Add this line
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.adapter_init_conv = nn.Conv2d(channels, init_dim, 7, padding=3)
        ...
        mid_dim = dims[-1]
        self.adapter = Adapter(init_dim, mid_dim, time_dim)
        #self.adapter = Adapter(dim, time_dim)

    def forward(self, x: Tensor, xm: Tensor, t: Tensor = None) -> Tensor:
        """Pass a tensor through the module."""
        x = self.init_conv(x)
        xm = self.adapter_init_conv(xm)
        t = self.time_mlp(t)
        ...
        adapter_output = self.adapter(xm, t)
        print(f"Adapter: {adapter_output.shape}")
        ...
        adapter_output = self.adapter(xm, t)
        print(f"Adapter: {adapter_output.shape}")
        if adapter_output.shape != x.shape:
            raise ValueError(f"Dimension mismatch: adapter_output shape {adapter_output.shape} does not match x shape {x.shape}")

        x = adapter_output + x
        print(f"After adapter: {x.shape}")
        x = downsample(x)
        print(f"After downsample 2 : {x.shape}")
...
# out_dim = None
# dim_mults = (1, 2, 4, 8)
# num_attention_layer = 5
# channels = 3
# block_groups = 8

# # Create an instance of the Unet class
# unett = Unet(dim, init_dim, out_dim, dim_mults, num_attention_layer, channels, block_groups)
# print(unett)
...
# adapter = Adapter(dim, time_dim)

# # Calculate the number of parameters
# num_params = sum(p.numel() for p in adapter.parameters())

# print(f'The Adapter class has {num_params:,} parameters.')
