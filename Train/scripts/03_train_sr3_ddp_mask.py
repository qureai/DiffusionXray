...
BASE_CONFIG: Final = {
    ...
    'model_params': {
        'dim': 16,
        'channels': 3,  # Updated channels to 3 to account for the segmentation mask
        'out_dim': 1,
...
def run(rank: int, world_size: int, cfg: Dict) -> None:
    ...
    train_dataset = MaCheXDataset(
        ...
        include_mask=True,  # Ensure the dataset returns the mask
    )
...
def train(
    ...
    for ep in range(ep_start, epochs + 1):  # type: ignore
        ...
        for batch in tqdm(train_loader, leave=False):
            ...
            # Concatenate noisy HR image with SR image and segmentation mask
            x_in = torch.cat([x_hr_noisy, x_sr, x_mask], dim=1)
